...
class OnsetMetric(Metric):
    ...
    def update(self, batch: MetricInput) -> None:
        cfg = self.config
        logits = batch.output.logits               # (B, n_classes) or (B, K*3+1)
        target = batch.target.target_bin           # (B,)
        pred = self._decode_pred(batch)            # (B,)
        stop_idx = cfg.b_pred
    ...
    def compute(self) -> dict[str, float]:
        ...
        out: dict[str, float] = {
            ...
            "onset/pred_stop_rate":    self._stop_pred_total / max(self._n_total, 1),
            "onset/pred_stop_fp_rate": self._pred_stop / n,
