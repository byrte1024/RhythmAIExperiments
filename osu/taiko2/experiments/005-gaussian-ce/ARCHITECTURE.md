# Experiment 005 — Gaussian soft-target CE · Architecture
...
the cursor to the next onset — or `STOP` if no onset falls within the
500-bin (≈ 2.5 s) prediction range.

One cursor per training sample. The cursor is placed one of three
ways (see **Sample construction**) and the target is derived from the
`d_events=1` next-event slot.
...
Out-of-window events (those with `token_pos < 0`) are ignored (no
virtual tokens).
...
The model emits one `(B, 501)` logit tensor. The loss routes those
logits two ways without any architectural split:

- **STOP logit** — `stop_logit = logits[:, 500]`, treated as a sigmoid
  binary classifier.
- **Bin logits** — `bin_logits = logits[:, :500]`, treated as a
  softmax distribution over the 500 non-STOP bins.

### Hyperparameters

| Param | Value |
|---|---|
| `sigma_bins` (Gaussian std. in bins) | 2.0 |

### Binary STOP BCE

```
stop_target = (targets == 500).float()        # (B,), 1 iff STOP
stop_bce    = BCE_with_logits(stop_logit, stop_target, reduction="mean")
```

Pulls `stop_logit` up on STOP samples and down on bin samples,
independently of any bin logit.

### Bin-only Gaussian softmax CE

Evaluated on the subset of samples whose target is a bin (i.e.
`targets != 500`). For each such sample with target `t`:

```
bins         = arange(0, 500)                              # (500,)
w_i          = exp(-0.5 * ((i - t) / sigma_bins)^2)        # unnormalized Gaussian
soft_target  = w / w.sum()                                 # renormalized row
log_probs    = log_softmax(bin_logits[non_stop], dim=-1).clamp(min=-100)
bin_ce_i     = -(soft_target * log_probs).sum(-1)          # per-sample CE
bin_ce       = bin_ce_i.mean()                             # over non-STOP samples only
```

If the batch is STOP-only, `bin_ce = 0` (no non-STOP samples to
average over) and only `stop_bce` contributes to the total loss.
...
logging: `loss`, `stop_bce`, `bin_ce`, `stop_rate` (= mean of
`is_stop` across the batch).

### Notes on the design

- **STOP cannot steal soft mass from bins.** STOP is a fully separate
  binary task at the loss level. The softmax used for bin CE only
  sees the 500 bin logits, so probability mass on non-STOP bins
  competes only with other bins.
- **Gaussian vs trapezoid.** With `sigma_bins = 2`, ~68 % of the soft
  target's mass lies within ±2 bins of the true target and ~95 %
  within ±4 bins; the tail decays smoothly and is never exactly zero,
  so very-wrong predictions still receive gradient proportional to
  distance (unlike a trapezoid that clips to 0 past its cutoff).
- **No hard-CE mix, no frame-tolerance floor, no STOP-weight
  multiplier.** `sigma_bins` is the only loss hyperparameter.
...
Not run as part of the training loop. The trained checkpoint writes
config snapshots sufficient for an inference script to reconstruct:
...
The argmax decoder takes `argmax(logits, dim=-1)` over the full 501
classes — STOP and bins are compared directly by raw logit magnitude,
matching inference-time behaviour for a model trained with this loss.
...
- With `allowed_overlap_forward = allowed_overlap_back = 0`, every
  event cursor is a valid sample. The `min_cursor_bin = 6000` filter
  drops early-silence cursors but the per-event expansion leaves the
  total sample count in the millions.
- **Subsample = 1 (full dataset).**
