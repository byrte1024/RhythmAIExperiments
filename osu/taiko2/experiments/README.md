...
| [017f](017f-framewise-bce-metrics-rerun/) | Framewise BCE metrics rerun | `Complete` | 1:1 rerun of #017e with upgraded metrics. Statistically identical training (F1 delta +/-0.006 from E3). New baselines: FPS-50 binary F1 0.650 (peaked E6), P/R/F1 at 25ms 0.778/0.757/0.771 (tau=0.4). Distributional metrics confirm tau=0.4 as structural optimum: gap_TVD 0.327, gap_peak_IoU 0.745, bpm_ratio 0.994. Silence regions weakest (overlap F1 0.546). fps50_f1 watched metric peaked at E15 (0.741), later than frame F1 (E11), capturing a different quality dimension. |
| [019](019-coincidence-input/) | Coincidence input (mel + 13-row summary) | `Complete` (rejected) | **Model ignores coincidence channels.** no_coincidence benchmark +0.2-0.4% at all 6 evals (needed >= -3%). Coincidence is redundant with mel -- derived from mel, so model extracts same info directly. Coincidence-only (no_mel) gets F1=0.119 (vs 0.033 with nothing), proving signal exists but is redundant. Small F1 gain at matched DR (+0.012 at tau=0.4) likely from wider conv stem (93 vs 80 channels), not coincidence. gap_TVD 0.294 new best. Silence_f1 did not improve. |
| [020](020-activation-maximization/) | Activation maximization (input dreaming) | `Complete` | Saliency reveals model uses high-frequency transient attack (bands 70-79 saliency 2-4x bands 0-9), not low-frequency body -- opposite of predicted low-band dominance. Confidence is bin-exact (peak at offset 0, no anticipation). Dense patterns (50 onsets/window) hit capacity wall (0.889 conf vs 0.968 at 13 onsets) due to Conv1D receptive field overlap. Event context negligible (delta < 0.02 conf). Conditioning density modulates selectivity: non-target conf drops to 0.012 at density_mean=8 vs 0.04 at density=3. |
| [022b](022b-dual-channel/) | Dual-channel mel + octopus | `Complete` | **New best model.** AR F1 0.794 (+0.012 over mel, best threshold). Beats mel at EVERY threshold (+0.009 to +0.036). At matched density: F1 0.785 (+0.016), gap_TVD 0.290 (best ever, -0.039), dc_human 92.79 (+0.28). Best dc_human ever: 93.26. Octopus contributes -0.7% when zeroed (real but small). Mel remains dominant (-40.7% when zeroed). Path to F1 >0.8 remains open -- gains are incremental input improvements; larger jumps likely require architecture/loss/multi-scale changes. |
| [022](022-octopus-input/) | Octopus gradient onset representation | `Complete` | Octopus-only plateaus at frame F1 0.784 (mel: 0.822), AR F1 0.728 (mel: 0.771). Learnable but 0.035-0.04 below mel. silence_f1 0.389 WORSE than mel's 0.527 -- log-domain loudness normalization causes hallucinations in quiet sections. Mini-to-AR gap larger (0.128 vs 0.100 at fps50) -- representation lacks energy context for AR coherence. Less context-dependent (no_context -7% vs -10.3%). Natural next step: dual-channel mel+octopus. |
| [021](021-wider-stem/) | Wider conv stem | `Complete` (rejected) | stem_width 192 -> 256 does not reproduce 019's gains in threshold sweep. Per-eval training showed consistent dc_human +0.4 and gap_TVD -0.02, but these wash out under threshold normalization. 3-way comparison: 019 still wins on F1 (+0.012), gap_TVD (-0.041), dc_human (+0.44) at matched density. Conv stem width is not the bottleneck; 019's improvement remains unexplained. |
| [020b](020b-legible-dreams/) | Legible audio dreams | `Complete` | Legible preset resolved #020's near-zero energy: onset energy 4.1 dB (was -0.004), mel range -12 to +37 dB (was +/-0.07), mel-confidence correlation +0.18 (was -0.05). Key refinement: saliency (high-band sensitive) and dreaming (low-band dominant, ratio 1.3-1.5x) measure different things -- model detects via high-band transient attack but constructs low-frequency-dominant onsets. Conditioning modulates background energy, not onset shape. Event context remains negligible (<0.1 dB delta). |
| [023](023-kind-acoustics/) | Kind acoustics and pattern analysis | `Complete` | Corpus analysis (10,048 charts, 6,934,185 events [023-kind-acoustics/results/summary.json]). **D and K are acoustically identical** — Fisher LDA 0.000105 on 6.5M onsets [same file]; a mel-based D/K classifier sits at chance. The corpus is 62/38 alternation-biased (an "always alternate" baseline scores 62 % on next-type). BIG/NORMAL separation is IOI-dominated (BIG has 2× longer preceding IOI, Fisher 90× higher than D/K but still <0.01) and BIG notes cluster (BIG→BIG ~19 %). Conclusion: D/K typing must be pattern-based, BIG from rhythmic position. Motivates the #024 typing model. |
| [024](024-typing-baseline/) | Typing model smoke test (subsample-16) | `Complete` | First typing model — predicts D/K type + BIG/NORMAL strength from onset-indexed windows of hits. Teacher-forced type accuracy **0.6983** [024-typing-baseline/metrics.json, step 18,354, val/single/typing/type/accuracy] beats #023's 62 % alternation baseline (+7.8 pp); strength best_f1_BIG 0.599, combined accuracy 0.6185. 5 of 6 pre-run predictions matched or beat. Architecture validated. |
| [024b](024b-typing-full/) | Typing full data + entropy penalty | `Complete` | Full dataset, 16/16 context, entropy penalty. Teacher-forced type accuracy plateaued at **0.7177** [024b-typing-full/metrics.json, step 489,480, val/single/typing/type/accuracy]; strength best_f1_BIG 0.7026, combined 0.6713. AR type_accuracy_sym 0.5424 — a 17.5 pp teacher-forced→AR gap. Type accuracy ceiling near 0.72 at 16/16 context. |
| [024c](024c-typing-ctx32/) | Typing context 32/32 | `Complete` | Doubling context (16/16 → 32/32) lifted teacher-forced type accuracy 0.718 → **0.7255** (+0.8 pp) [024c-typing-ctx32/metrics.json, step 489,480, val/single/typing/type/accuracy]; strength best_f1_BIG 0.7191, combined 0.6823, AR type_accuracy_sym 0.5556 (+1.3 pp). The 16→32 jump is the significant one. |
| [024d](024d-typing-ctx64/) | Typing context 64/64 | `Complete` | **Best typing model.** Teacher-forced type accuracy **0.7279** [024d-typing-ctx64/metrics.json, step 489,480, val/single/typing/type/accuracy] (+0.2 pp over #024c); strength best_f1_BIG 0.7263, combined 0.6866, AR type_accuracy_sym 0.5618. The 32→64 jump sits within noise — context scaling concluded at 32/32 as the sweet spot. |
| [025](025-typing-temporal-bias/) | Typing temporal-distance bias | `Complete` (hypothesis rejected) | Learned temporal-distance attention bias on the 32/32 typing model. Type accuracy peaked at **0.7149** [025-typing-temporal-bias/metrics.json, step 293,688, val/single/typing/type/accuracy] — 1.1 pp below target, ~0.7 pp behind #024c, consistently ~1 pp behind across all 12 evals. AR type_accuracy_sym peaked 0.5464 (1-2 pp worse). Learned sigma exploded to 207 bins. Temporal bias does not lift the ~0.728 ceiling. |
