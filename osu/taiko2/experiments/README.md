...
| [011](011-onset-feature-survey/) | Survey 8 classical onset detection algorithms against `taiko2_v1` GT, frame-wise P / R / F1 across tolerances and threshold sweeps | `Planned` | Analysis (no training). Characterizes which ODFs have highest recall on chart-author-chosen onsets, which combinations maximize joint coverage, and informs the channel set for the planned input-feature training experiment (#012). |
