...
| [017d](017d-framewise-bce-noweight/) | Framewise BCE no pos_weight (symmetric) | `Complete` | **First framewise model to beat #007 on AR corpus metrics.** Threshold sweep found optimal at E9 (step 186,066) tau=0.3: `matched_rate` 0.742 vs #007's 0.703, `hallucination_rate` 0.118 vs #007's 0.172 [threshold_sweep.json]. `dc_human` 92.1 [exp_017d_framewise_bce_noweight, step 186,066], `halluc_rate` 0.118 (vs #007's 0.172), `density_ratio` 0.925. Symmetric BCE produces precision-first training (precision 0.92 at E1, recall climbs +0.13 across 10 evals). Model is 99% audio-driven (no_future_audio F1=0.000, no_context drops only 5%). Confidence range compressed (TP median ~0.74) but well-calibrated (ECE 0.004). Loss-optimal checkpoint (E2) is the worst for AR — `metric_to_watch` should not be loss. Overfitting from E3 limits ceiling. |
