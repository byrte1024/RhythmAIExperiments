...
| [022](022-octopus-input/) | Octopus gradient onset representation | `Planned` | Drop-in mel replacement with biologically-inspired octopus cell onset representation (Golding et al. 1995). Gammatone filterbank + cross-frequency synchrony detection + group delay compensation. 97-cell gradient at 200 fps. Dataset `taiko2_v1_mel_octopus` stores mel+octopus (177 channels); `feature_rows` selects octopus-only (rows 80-176) at training and inference. Tests whether pre-computed onset synchrony outperforms learned mel-based onset detection. |
