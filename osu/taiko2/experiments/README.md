...
| [011b](011b-onset-disagreement/) | Pairwise disagreement / complementarity matrix + per-kind / per-density / per-star sub-analyses on the same algorithms as #011 | `Complete` | **Pre-run "redundancy dominates" hypothesis rejected.** Best 2-channel union (`hfc_mel + spectral_flux`) hits **recall 0.905, +16.3 pp** over best single channel. Cross-group pairs (envelope-based + difference-based) consistently top the table (J 0.57-0.69, marg 11-13 pp); within-group pairs near-redundant. F1 still peaks at K=1; recall keeps climbing through K=3 (0.932). **Sub-band DON/KA specialization not observed** — abandon the sub-band-as-channel design. Per-density and per-star F1 *inverse* the prediction: sparse / easy charts have low precision (many unmapped audio onsets), dense / insane have high. Recommended channel set for #012: SF + HFC (or + energy). |
| [012](012-onset-channels/) | Append 4-band sub-band spectral flux as extra mel rows; everything else identical to #007 | `Planned` | First training experiment to use a non-mel input feature. New `mel_onset` audio sampler appends 4 sub-band-SF rows to the existing 80 log-mel bands (range-matched to dB scale); model conv stem expands to read 84 input rows. Predicted: val miss 0.220-0.232 (vs #007 best 0.241), 1-2 pp improvement. Failure modes (channel over-reliance, precision floor at strict tolerances) tracked. |
