# Experiment 001 — exp 45 port, subsample-16 smoke · Architecture
...
| Min cursor bin filter | 0 (smoke run; taiko1 default is 6000) |
...
| **Subsample** | **16 (keep every 16th sample after overlap filtering)** |
...
| Evals per epoch | 4 |
| Watched metric | `onset/bad` (lower is better) |
| Checkpoint cadence | every eval — `latest.pt` rewritten; `best.pt` on new best |
| Step log cadence | every step into `metrics.jsonl` |
| Eval artifacts | scatter / distributions / ratio_error / error_hist PNG + .npz under `{run_dir}/eval_{step}/` |
| Seed | 42 |
...
- `onset/hit`, `onset/good`, `onset/bad`
- `onset/ihit`, `onset/igood`, `onset/ibad` (only when `all_future_bins` has ≥ 1 real event)
...
- Raw training samples after overlap filter (500 / 500): ≈ 388 k.
- **After `subsample = 16`: ≈ 24 k training samples**.
- Val samples (no subsample): ≈ 39 k.
