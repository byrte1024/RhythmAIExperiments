# Experiment 010d — Ratio decomposition with shared gradients · Architecture
...
**Soft expectations (NOT detached — gradients shared with ratio head):**
```
div_val = Σ_i softmax(div_logits)_i × i    → (B, 1), GRADIENT FLOWS
off_val = Σ_i softmax(off_logits)_i × i    → (B, 1), GRADIENT FLOWS
div_emb = Linear(1, 384)(div_val)
off_emb = Linear(1, 384)(off_val)
```
**This is the only architectural change from #010.** The ratio loss
backprops through the soft expectations into the divisor and offset
heads, so the aux heads are jointly trained by their own CE losses
AND the ratio loss gradient.
...
unreliable (< 2 past events or no clear peak). **In #010d the
divisor head ALSO receives ratio_loss gradient via shared
expectation backprop** — it is jointly trained by div_ce + the
ratio loss path through `div_val`.
