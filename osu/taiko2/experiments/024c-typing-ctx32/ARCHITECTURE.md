# Architecture -- Experiment 024c: Typing model with context 32/32
...
| Future context dropout | per sample | 10 % | Mask all 32 future tokens as padded. Forces prediction from past context alone. |
...
Bidirectional transformer encoder over 65 onset tokens.
...
Same as 024b: TypingMetric (type P/R/F1, strength P/R/F1, combined
...
`type_chart` function reads `past_context` and `future_context` from
`model.config`, so AR inference automatically uses the 32/32 window.

## Inference

Same spec pattern as 024b. `config/infer.json` points to the
