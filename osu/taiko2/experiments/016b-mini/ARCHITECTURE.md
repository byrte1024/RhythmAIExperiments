# Experiment 016b-mini — Min-SNR sign probe · Architecture
...
| Subsample | 8 (keep every 8th training sample — probe-only; #016b proper uses 1) |
...
| `snr_gamma` | `5.0` |
| `snr_x0_mode` | `true` (x0-parameterization form: `w = min(snr, γ)`; the canonical formula from Hang et al. 2023 §4.2 for x0-prediction networks. Inverts the per-t weight direction relative to #016, which ran with `snr_x0_mode=false` and applied the ε-prediction form `w = min(snr, γ) / snr`.) |
...
# Min-SNR-γ weighting per sample (Hang et al. 2023 §4.2, x0-mode).
# `snr_x0_mode=true` selects the x0-parameterization form: no
# division by snr. At low t (high SNR) the weight saturates at γ=5
# so the refinement-regime gradient is preserved; at high t (low
# SNR) the weight equals snr and decays naturally to ~0.
snr_t          = ab_cum(t) / (1 - ab_cum(t))            # (B,)
snr_w          = min(snr_t, 5.0)                        # (B,) — x0-mode form
...
| Batch size | 64 |
| Epochs | 2 (probe cap; expected manual stop at eval 1 or eval 2) |
...
- **Subsample = 8 (1/8 of the training samples; keep every 8th).** Probe-only — #016b proper restores `subsample = 1`.
