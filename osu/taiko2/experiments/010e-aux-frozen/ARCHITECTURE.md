# Experiment 010e — Frozen aux heads after extended warmup · Architecture
...
| Name | Shape | Dtype | Description |
|---|---|---|---|
| packed | `(B, 856)` | float32 | `[div_logits(500) | off_logits(100) | ratio_logits(256)]` |

Parsed as:
```
div_logits   = output[:, :500]          # (B, 500) — divisor head
off_logits   = output[:, 500:600]       # (B, 100) — offset head
ratio_logits = output[:, 600:856]       # (B, 256) — 255 ratio bins + STOP
```
...
ratio_input = cursor_tok + div_emb + off_emb
  ...
  → ratio_logits (B, 256)   # 255 ratio bins + 1 STOP
```
...
255 bins log-spaced from 0.125× to 8.0× (6 octaves):
```
centers = exp(linspace(log(0.125), log(8.0), 255))
```
- 42 bins per octave, ~1.65% resolution per bin.
- bin 0 = 0.125×, bin 85 ≈ 0.5×, bin 127 = 1.0×, bin 169 ≈ 2.0×,
  bin 254 = 8.0×.
- bin 255 = STOP (the 256th class).
- All standard musical ratios (1/8, 1/6, 1/4, 1/3, 1/2, 1, 2, 3, 4,
  6, 8) are within 0.7% of a bin center.
...
| aux_weight | 0.1 |
| stop_weight | 1.5 |
| ratio_freeze_evals | 8 |
| freeze_aux_after_warmup | true |
...
### Warmup and aux freeze

For the first `ratio_freeze_evals = 8` evals (~165,392 steps at
`evals_per_epoch = 4`, 20,674 steps per eval), the ratio CE is
zeroed and the ratio HIT/GOOD/MISS metrics are skipped — only the
divisor and offset heads train. The model side mirrors this: while
in training mode and below the warmup step count, the ratio MLP
and Conv1d smoothing layers are not executed (the ratio block of
the output is filled with zeros), saving forward and backward
compute on the ratio head during warmup.

At the warmup boundary (the first training-mode forward where
`fwd_step >= warmup_step_limit`), `freeze_aux_after_warmup = true`
triggers `requires_grad = False` on the parameters of the divisor
head, the offset head, the divisor-value embedding
(`divisor_val_emb`), and the offset-value embedding
(`offset_val_emb`). After this point, only the backbone, the
ratio MLP, and the Conv1d smoothing receive gradient updates;
`div_ce` and `off_ce` are still computed and reported but
contribute no parameter updates to the heads (their gradients land
on parameters with `requires_grad=False` and are discarded by the
optimizer step).

The freeze is one-way and lazy — it fires once on the first
post-boundary forward and is never undone within the run. Eval-mode
forwards do not increment the step counter, so the boundary is
reached strictly via training steps.
