# Architecture — Experiment 024: Typing model smoke test
...
- Subsample: 8 (every 8th sample kept, for smoke test speed).
...
Two augmentations, applied in the adapter during batching (training
only):

1. **D/K flip** (probability 0.5): swap all D/K labels in the window
   — every past kind label 0 becomes 1 and vice versa, and the
   target's GT label is flipped correspondingly. Strength labels are
   unchanged. Enforces the corpus-wide D/K symmetry prior: a valid
   chart with all D/K swapped is equally valid.

2. **Future context dropout** (probability 0.1): mask all 16 future
   onset tokens as padded. Forces the model to predict from past
   context alone, improving robustness for chart-boundary and
   early-chart inference.
...
Two independent binary cross-entropy terms:

```
type_loss = BCEWithLogits(type_logit, type_target)
strength_loss = BCEWithLogits(strength_logit, strength_target,
                              pos_weight=17.0)
total_loss = type_loss + strength_loss
```
...
```
loss, type_loss, strength_loss,
type_acc, strength_acc, combined_acc,
type_entropy_mean, strength_entropy_mean
```
...
- Epochs: 3 (smoke test).
- Batch size: 128.
- Evals per epoch: 2 (6 evals total).
- Seed: 42.
...
## Inference (not used in 024 smoke, documented for 024b)

The typing model runs as a second pass after the onset detector:

1. Onset detector produces onset positions (bins).
2. For each onset in order, the typing model receives:
   - Past 16 onsets with their predicted D/K + big labels.
   - Target onset with UNKNOWN labels.
   - Future 16 onsets with positions/IOIs only, UNKNOWN labels.
3. Predict D/K and big/normal for the target onset.
4. Add prediction to history, advance to next onset.

500 forward passes of a 171K-param model on 33 tokens each = well
under 1 second per chart on GPU.

## Environment

- Python 3.13, PyTorch nightly (from pyproject.toml + uv.lock).
- Training on CUDA (smoke test) or CPU.
