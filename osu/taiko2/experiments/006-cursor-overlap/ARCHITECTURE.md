# Experiment 006 — Cursor-overlap filtering · Architecture
...
ways (see **Sample construction**) and the target is derived from
the `d_events = 1` next-event slot.
...
| **Allowed overlap forward** | **500 bins** (this experiment's change) |
| **Allowed overlap backward** | **500 bins** (this experiment's change) |
| Past-event padding | Start (oldest-first, back-aligned) |
| Future-event padding | End |
| **Subsample** | **1 (full dataset)** |

**Overlap-gap semantics.** After iterating event cursors in chart
order, a cursor is kept iff it is at least
`max(allowed_overlap_forward, allowed_overlap_back) = 500` bins past
the previous kept cursor. With `a_bins = b_bins = 500`, this means
consecutive kept samples have **non-overlapping** directional audio
windows. Sample count drops by roughly an order of magnitude vs the
unfiltered (overlap = 0) regime.
...
Applied to each sample via a post-sample pipeline.
...
**Total parameters: ≈ 16.5 M** (exact value printed on
`model.n_params` at construction; verified in unit tests in range
15–18 M).
...
feature from these concatenated parts (5 × 384 = 1920 total,
projected back down to 384):

1. **Presence** — a single learned parameter `(1, d_model)`
   broadcast to every slot.
2. **`gap_before[i]`** — `|event_offsets[i] - event_offsets[i-1]|`,
   clamped to ≥ 1; for `i = 0` a placeholder value of 50 is used.
   Encoded via sinusoidal position embedding at `d_model`.
3. **`gap_after[i]`** — `|event_offsets[i+1] - event_offsets[i]|`,
   clamped to ≥ 1. For the last valid event in each row `gap_after`
   would span the target gap; we overwrite it with that event's
   `gap_before` as a structure-preserving proxy.
4. **`gap_ratio_before[i]`** — `gap_before[i-1] / gap_before[i]`,
   clamped to `[0.1, 10.0]`, multiplied by 50. Sinusoidal-encoded.
5. **`gap_ratio_after[i]`**  — `gap_after[i+1] / gap_after[i]`,
   same clamp + 50× scale. Sinusoidal-encoded.
...
Out-of-window events (those with `token_pos < 0`) are ignored.
...
Returned `LossResult.metrics` carries four detached floats for
logging: `loss`, `hard_ce`, `soft_ce`, `stop_rate`.
...
| **Evals per epoch** | **1** (this experiment's change; reflects the ~10× smaller post-filter epoch) |
...
Computed by `OnsetMetric` on the full val pass:

- `onset/exact`, `onset/fhit`, `onset/fgood`, `onset/fmiss`
- `onset/rhit`, `onset/rgood`, `onset/rmiss`
- `onset/hit`, `onset/good`, `onset/miss`
- `onset/ihit`, `onset/igood`, `onset/imiss` (only when
  `all_future_bins` has ≥ 1 real event)
...
- `onset/pred_stop_rate` — **total STOP predictions / total samples**
  (fixed in this experiment; the legacy FP-only quantity is kept as
  `onset/pred_stop_fp_rate`)
- `onset/pred_stop_fp_rate` — FP STOP predictions / non-STOP sample
  count (legacy; was called `pred_stop_rate` in #002 / #005)
...
### Auxiliary eval passes (new in #005, carried into #006)

- **`train_noaug`** — evaluate 25 % of the **train** split with
  augmentations OFF after each eval. Metrics prefixed
  `val/single/train_noaug/*` so the overfit-gap-vs-val is one
  subtraction away. Same `OnsetMetric`.
- **`benchmarks`** — 10 input-distortion modes on 25 % of val per
  eval (raised from the 5 % default because val is ~10× smaller
  after overlap-filtering). Modes: `normal`, `no_audio`, `no_future_audio`,
  `no_past_audio`, `static_audio`, `no_context`, `random_context`,
  `metronome`, `advanced_metronome`, `time_shifted`. Metrics
  prefixed `val/single/bench/{mode}/*`.
- **AR corpus inference hook** — every eval runs AR inference on
  10 % of val charts using the LIVE model (no checkpoint reload),
  both `gt` and `fixed` conditioning modes. Averaged
  `matched_rate`, `dc_human`, `hi_pspace`, `error_median_ms`,
  `density_ratio`, etc. merged into val metrics under
...
Not run as part of the training loop's main eval pass (but the
corpus hook does run AR on 10 % of val charts). The trained
checkpoint writes config snapshots sufficient for an inference
script to reconstruct: `EventEmbeddingDetector` +
`ArgmaxDecoder(b_pred=500)` +
...
2. While `cursor < max_bin` and `step < max_events`:
   ...
   - `STOP` (empty `bin_offsets`) → `cursor += hop_bins_on_stop`
     (default 20).
   - Else → emit onset at `cursor + bin_offset`, advance `cursor`
     there.
...
- **With `allowed_overlap_forward = allowed_overlap_back = 500`,
  consecutive kept cursors are ≥ 500 bins apart — non-overlapping
  directional audio windows. Sample count drops by ~10× vs the
  overlap-0 regime used in #002/#005.**
- `min_cursor_bin = 6000` drops early-silence cursors.
- **Subsample = 1 (full dataset).**
