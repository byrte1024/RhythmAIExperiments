# Experiment 44e — Full Architecture Specification
...
## Loss: 3-Component OnsetLoss

```
loss = 0.5 * hard_CE + 0.5 * soft_CE + 2.5 * distance_ramp
```

### Component 1: Hard CE (hard_alpha = 0.5)

Standard cross-entropy against the exact target bin. Provides precise gradient only at the target.

### Component 2: Soft CE (1 - hard_alpha = 0.5)

Trapezoid soft targets in log-ratio space:
...
### Component 3: Distance Ramp (ramp_alpha = 2.5, ramp_exp = 1.0)

Log-ratio distance penalty that provides gradient for far-off predictions:

```
probs = softmax(logits)                           # (B, 501)
expected_pred = sum(probs * bins)                  # model's center of mass
log_dist = |log((expected_pred + 1) / (target + 1))|  # ratio-space distance
ramp = 2.5 * log_dist^1.0                         # linear in log-ratio space
```

Properties:
- 0 at exact match
- Symmetric in ratio space: predicting 2x over = same penalty as 0.5x under
- Proportional across all target values
- Differentiable everywhere via softmax center of mass
- Only applied to non-STOP targets

This component solves the flat plateau problem: without it, predicting 2x and 4x the target produce the same loss. With it, 4x gets higher loss than 2x, giving the model gradient to improve from "very wrong" to "less wrong."
