# Experiment 50 — Full Architecture Specification
...
An explicit entropy penalty added to the loss to force the model to produce sharper, more committed softmax distributions:

```
probs = softmax(logits)  # (B, 501)
entropy = -sum(probs * log(probs + 1e-10), dim=-1)  # (B,) per-sample entropy in nats
anti_entropy = mean(entropy)  # scalar, averaged over batch
```

### Combined Loss

```
loss = onset_loss + 0.1 * anti_entropy
```

The entropy weight of 0.1 adds approximately 0.23 to the loss (~8% of onset loss). This is enough to sharpen predictions without causing mode collapse.
...
## Key Metrics (eval 21, converged)

| Metric | Value |
|---|---|
| HIT (<=3% or +/-1 frame) | 72.9% |
| MISS (>20% off) | 26.5% |
| Exact match | 54.1% |
| AR step0 | 73.7% |
| Metronome benchmark | 45.5% |
| Adv metronome | 52.0% |
| Time shifted | 48.3% |
| stop_f1 | 0.534 |
| Unique predictions | 442 |
