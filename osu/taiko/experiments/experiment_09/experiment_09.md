...
A critical correction came during analysis of these results: low accuracy on metronome and time_shifted benchmarks does NOT mean the model is ignoring the fake events — it means the model IS following them. If the model used audio to override the fake events, it would score around 26% (the no_audio baseline). Scoring 5.5% means the fake events actively drag the model away from audio-based predictions. The model trusts events over audio even when events are completely wrong.
