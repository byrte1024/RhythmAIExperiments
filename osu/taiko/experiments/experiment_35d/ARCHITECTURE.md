# Experiment 35-D — Full Architecture Specification
...
## Model: OnsetDetector (with exponential decay mel ramps + amplitude jitter + focal loss)

**Total parameters: ~19M**

Same unified OnsetDetector architecture: exponential decay ramps (sharp spikes at event positions) are embedded into the mel spectrogram, then processed through AudioEncoder (conv stem + 4 transformer layers), GapEncoder (2 transformer layers), and a 4-layer Fusion Transformer. The cursor token at position 125 feeds the output head. The only change from the base architecture is adding focal loss (gamma=3.0).
...
## Loss: OnsetLoss (with focal modulation, gamma=3.0)

Mixed hard CE + soft target loss with focal modulation.
...
```
ce = 0.5 * hard_CE + 0.5 * soft_CE
pt = exp(log_probs[target])   # probability of correct class
focal_loss = (1 - pt)^3.0 * ce   # gamma=3.0, aggressively downweights easy samples
```
...
| Metric | Value |
|---|---|
| HIT (<=3% or +/-1 frame) | 64.0% |
| MISS (>20% off) | 35.4% |
| Context delta | 7.1% |
| no_events accuracy | 39.1% |
| Unique predictions | 458 |
