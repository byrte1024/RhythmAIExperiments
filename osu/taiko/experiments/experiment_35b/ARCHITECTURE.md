# Experiment 35-B — Full Architecture Specification
...
## Model: OnsetDetector (with full-band linear mel ramps)

**Total parameters: ~19M**

Unified fusion architecture (AudioEncoder + GapEncoder + FusionTransformer, single cursor-token output). This experiment uses full-band linear mel ramps instead of edge-band ramps.

### Mel-Embedded Event Ramps (full-band linear)

Before audio encoding, linear gradient ramps are embedded across ALL 80 mel bands with halved audio:

```
mel = mel * 0.5 + ramp * 10.0   (broadcast across all 80 bands)

For each pair of consecutive events (and from last event to cursor):
  Linear ramp from 1.0 at event position to 0.0 at next event
  Added uniformly to all 80 mel bands (no per-band fading)
  Audio scaled by 0.5 before addition
```

The conv stem cannot separate ramps from audio because the ramp signal is uniform across all frequency bands.
...
## Key Metrics (ATH at eval 3)

| Metric | Value |
|---|---|
| HIT (<=3% or +/-1 frame) | 69.5% |
| MISS (>20% off) | 30.0% |
| Context delta | 5.4% |
| no_events accuracy | 45.2% |
| Unique predictions | 444 |
