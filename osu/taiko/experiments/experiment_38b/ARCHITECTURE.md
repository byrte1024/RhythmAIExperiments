# Experiment 38-B — Full Architecture Specification

## Task

Predict onset probability at every position in the future audio window. Per-token framewise detection with weighted BCE loss.
...
FramewiseOnsetDetector architecture (~12M params) predicting per-token onset probabilities for each of the 125 future tokens. Two changes from the initial framewise design: (1) teacher forcing removed, (2) causal mask corrected so past tokens cannot see future tokens.
...
### 3. Self-Attention with Fixed Causal Mask (6 layers)

Corrected causal mask:
- Past tokens (0-124): see only other past tokens (blocked from future)
- Future tokens (125-249): see all past + causal within future
...
No onset feedback / teacher forcing.
...
```
loss = F.binary_cross_entropy(probs, target, weight=sample_weights)
```

pos_weight=7.0 for onset tokens (positive class), 1.0 for non-onset tokens. With ~13% natural positive ratio, 7x balances the classes.
...
- Multi-target labels: binary onset mask at token resolution
