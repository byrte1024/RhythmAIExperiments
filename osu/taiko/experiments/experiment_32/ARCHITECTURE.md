# Experiment 32 — Full Architecture Specification
...
## Model: DualStreamOnsetDetector (with audio skip connection)

**Total parameters: ~23.3M**

Two independent streams process audio and context in parallel, then exchange information via 2 bidirectional cross-attention layers in the final layers. A skip connection on the cursor token preserves fine-grained audio features through the fusion stage.
...
```
# save pre-fusion audio cursor for skip connection
...
### 5. Output Head (with skip connection)

```
cursor = audio[:, 125, :] + audio_pre_cursor   (B, 384)  # skip connection preserves fine-grained audio
...
The skip connection ensures the output head always sees both:
- Fine-grained audio features (pre-fusion cursor) for precise onset timing
- Context-informed audio features (post-fusion cursor) for pattern awareness
...
| Metric | Value |
|---|---|
| HIT (<=3% or +/-1 frame) | 64.1% |
| MISS (>20% off) | 34.4% |
| Context delta | -1.8% |
| no_events accuracy | 45.3% |
| Unique predictions | 363 |
