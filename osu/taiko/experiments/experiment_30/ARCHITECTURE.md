# Experiment 30 — Full Architecture Specification
...
Processes inter-onset intervals (gaps) from event offsets.
...
x = gap_features + snippet_feat + Embedding(C+1, 384)(seq_pos)
```
...
Concatenates audio and gap tokens, then runs self-attention over all.

```
x = concat([audio_tokens, gap_tokens], dim=1)   → (B, 250+C, 384)
mask = concat([zeros(B, 250), gap_mask], dim=1)  → (B, 250+C)
...
Applied after conv stem, after every AudioEncoder layer, every GapEncoder layer, and every fusion layer:
```
...
### Cursor-Region Audio Masking (new in this experiment)
| Aug | Rate | Params |
|---|---|---|
| Cursor-region masking | 20% | Zero out 100-300 mel frames centered on cursor position 500 |
...
Killed after eval 2 (context delta collapsing).

| Metric | Eval 2 |
|---|---|
| HIT (<=3% or +/-1 frame) | 67.9% |
| MISS (>20% off) | 31.6% |
| Context delta | 3.3% |
| no_events accuracy | 46.1% |
