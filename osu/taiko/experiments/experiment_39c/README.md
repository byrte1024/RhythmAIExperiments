...
Exp [39-B](../experiment_39b/README.md) showed global proximity reranking only gains +0.9pp because it regresses 1,299 correct predictions. The fix: only apply proximity bias when the model is uncertain.
...
**Barely better than [39-B](../experiment_39b/README.md) — entropy weight doesn't help.**
...
The entropy weight adds only +0.1pp over [39-B](../experiment_39b/README.md)'s +0.9pp. The regression ratio (~2:3) is unchanged. All weight combinations cluster around the same +0.9-1.0pp ceiling.
...
- **The [14.9pp theoretical ceiling](../experiment_39/README.md) requires oracle knowledge** (which candidate is nearest to cursor), not available from the model's output alone.
