...
Exp [19](../experiment_19/README.md) showed the gap-based context architecture is correct - first experiment where context beat audio during training, and [delta reached -0.18pp](../experiment_19/README.md) (best ever). But context plateaued around "slightly harmful" because it spent early epochs learning to rerank garbage proposals from an untrained audio model.
...
**1. Warm-start audio from exp [14](../experiment_14/README.md) best checkpoint**

Load `audio_encoder`, `event_encoder`, `audio_path`, `cond_mlp` from exp 14's best.pt ([epoch 6, 69.2% HIT, 50.5% accuracy](../experiment_14/README.md)). Context gets competent proposals from batch 1 - no garbage phase.

**2. Freeze audio components**

All audio components frozen (`requires_grad=False`). Only the 2.5M context path trains. Benefits:
- Audio is already at its best ([69.2% HIT](../experiment_14/README.md)) - no risk of degradation
...
If no candidate is within ±1 frame of the true target (and it's not STOP), that sample's selection loss is zeroed. Removes impossible training signal. At [exp 14](../experiment_14/README.md)'s quality (~97.5% target in top-K), this filters ~2.5% of samples.

### Architecture

Identical to exp [19](../experiment_19/README.md) - gap-based context with own encoders.
...
1. **Audio HIT = 69.2%** - frozen, should be exactly [exp 14](../experiment_14/README.md)'s performance.
...
4. **Override F1 improving over epochs** - unlike exp [19](../experiment_19/README.md) where it declined, stable proposals mean context can accumulate learning.
5. **Faster convergence** - 2.5M params, no audio backward pass.

### Risk

- Audio frozen at exp [14](../experiment_14/README.md) means the combined model can never exceed what exp 14's audio + context can achieve together. If the audio model's top-K doesn't contain good candidates for a segment, context can't help.
- Exp [14](../experiment_14/README.md) was trained with the legacy architecture (no top-K reranking). Its audio weights were optimized without a context path. The shared encoders may produce representations that work well for the audio path but aren't ideal for context's snippet extraction.
...
| Metric | [Exp 19 E1](../experiment_19/README.md) | Exp 20 E1 |
|--------|-----------|-----------|
...
**What worked:**
- Warm-start: Audio HIT at 69.5% from step 1 (matching [exp 14](../experiment_14/README.md)). No garbage phase.
...
**What didn't work:**
- Delta actually worse (-1.18pp vs [exp 19's -0.89pp](../experiment_19/README.md)). More overrides, but extra overrides are more wrong than right (false_topK 5.8% > true_topK 4.6%).
