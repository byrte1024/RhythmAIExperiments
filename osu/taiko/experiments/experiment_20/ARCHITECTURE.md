# Experiment 20 — Full Architecture Specification

## Task

Predict the next onset timing in an osu!taiko rhythm game chart, given audio + past event context. Autoregressive: each prediction advances the cursor to the predicted onset position. Context path reranks audio's top-K candidates using gap-based rhythm features.
...
Two-path architecture: a frozen audio path produces 501-way logits, a trainable context path reranks the top-20 candidates. Audio components warm-started from a pretrained LegacyOnsetDetector checkpoint (69.2% HIT) and frozen.
...
Candidates are NOT shuffled in exp 20 (shuffling introduced in exp 22).
...
Selection loss with hard cross-entropy on the K-way output.

### Selection Loss

```
target_in_topk = find candidate closest to true target (within ±1 frame)
if no candidate matches (target not in top-K): loss = 0 (masked out)
else: loss = CrossEntropy(selection_logits, best_candidate_idx)
```

Audio loss: none (audio path is frozen, no audio loss computed).

### Selection Loss Masking (new in exp 20)

If the true target is not within ±1 frame of any top-K candidate, the sample's selection loss is zeroed. At exp 14's quality level (~97.5% target in top-K), this filters ~2.5% of samples. Removes impossible training signal.
...
| Delta | -1.18pp |
| Override rate | 11.1% |
| Override accuracy | 41.6% |
| Override F1 | 22.1% |

## Context Status

Context was bolder than all prior experiments (11.1% override rate, 4.6% true_topK, 22.1% override F1 — all records at the time). But extra overrides were more wrong than right (false_topK 5.8% > true_topK 4.6%), producing net-negative delta. Warm-start and freeze validated as infrastructure, but the loss function (hard CE on selection) was identified as the bottleneck.
