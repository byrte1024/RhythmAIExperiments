# Experiment 47-D — Full Architecture Specification (Failed)
...
## Model: EventEmbeddingDetector (with binary_stop=True, forward-pool gate)

**Total parameters: ~16.1M**

Base architecture is EventEmbeddingDetector (single-pathway: learned event embeddings added to audio tokens at event positions, unified self-attention) with binary STOP gate. The gate reads from mean-pooled forward tokens instead of the cursor token.
...
### 5. Gate Head (Forward-Pool Binary STOP)

Instead of reading from the cursor token, the gate reads from mean-pooled forward tokens (positions 126-249, representing the future audio window):

```
forward_tokens = x[:, 126:250, :]  # (B, 124, 384)
forward_pool = mean(forward_tokens, dim=1)  # (B, 384)
gate_logit = Linear(384, 1)(LayerNorm(384)(forward_pool)) → sigmoid → P(stop)
```

Rationale: the STOP decision requires looking at the whole forward region ("is there any onset ahead?"), not just the cursor position.

Gate loss: balanced focal BCE (same as 47-C):
```
...
## Loss: OnsetLoss + Balanced Focal BCE Gate Loss
...
The forward-pool approach did not help. Mean-pooling 124 abstract tokens is too lossy, and gate_weight=2.0 still makes the gradient negligible.

## Summary of the Binary STOP Head Series (47, 47-B, 47-C, 47-D)

The binary head approach is fundamentally flawed for this problem. After 4 iterations, every variant failed:

| Exp | Fix Attempted | Result |
|---|---|---|
| 47 | Separate binary gate head | pos_weight backwards, STOP rate 0% |
| 47-B | Flipped targets + focal loss | focal .mean() drowns STOP, F1=0.066, recall=3% |
| 47-C | Balanced focal averaging | gate_weight too low, cursor token not suitable, F1=0.15 |
