...
All previous experiments ([05](../experiment_05/README.md)-[13](../experiment_13/README.md)) trained on a dataset with a fundamental timing misalignment: `BIN_MS` was hardcoded to `5.0ms` but the actual mel frame duration is `HOP_LENGTH / SAMPLE_RATE * 1000 = 110 / 22050 * 1000 = 4.98866ms`. This 0.01134ms-per-frame error compounds over song duration:
...
The dataset has been regenerated with the exact `BIN_MS = HOP_LENGTH / SAMPLE_RATE * 1000`. Everything else is identical to experiment [13](../experiment_13/README.md): same two-path architecture (~21M params), same loss (main + 0.2 audio aux), same AR augmentations (recency-scaled jitter, global shift, 8% insertions/deletions), same NaN guard.
...
- Miss rate dropping significantly, estimated **10-30%** (from exp [13](../experiment_13/README.md)'s ~43%)
...
This is the most impactful single change in the experiment series. Every model improvement from exp [05](../experiment_05/README.md)-[13](../experiment_13/README.md) was fighting against corrupted ground truth.
...
### vs Previous Best ([Exp 11](../experiment_11/README.md) E5)

| Metric | [Exp 11](../experiment_11/README.md) E5 | Exp 14 E1 | Exp 14 E8 |
|--------|-----------|-----------|-----------|
...
Every record broken. Miss rate went from ~42% to 30.3% (within the predicted 10-30% range). E1 alone already surpassed exp [11](../experiment_11/README.md)'s best epoch on every metric.
...
- **Why the model relied on events over audio (exp [09](../experiment_09/README.md))**: Audio was misaligned garbage past 30s. The model was *correct* to prefer events - they preserved relative spacing even when absolute positions drifted.
- **Why heavy augmentation was catastrophic (exp [07](../experiment_07/README.md))**: We corrupted the only reliable signal (events) while the "reliable" signal (audio) was broken. The model had nothing left.
- **Why the audio aux loss was load-bearing (exp [12](../experiment_12/README.md))**: Without forcing it, the audio path wouldn't learn because the audio genuinely didn't match the labels.
