# Experiment 47-C — Full Architecture Specification (Failed)
...
### 5. Gate Head (Binary STOP with Balanced Focal Loss)
...
Gate target: 1=stop, 0=onset.

Gate loss: balanced focal BCE:
```
focal_onset = mean(focal_bce for onset samples)
focal_stop = mean(focal_bce for stop samples)
gate_loss = (focal_onset + focal_stop) / 2
```

This ensures a batch of 997 onsets + 3 STOPs produces gate_loss where STOP contributes 50%, not 0.3%.
...
### Gate Loss (Balanced Focal BCE)
Separate averaging of STOP vs onset focal losses, then combined:
```
gate_loss = (mean_focal_onset + mean_focal_stop) / 2
```

Added gate loss (gL) and stop F1 (sF1) to tqdm training bar for monitoring.
