# Experiment 35-C — Full Architecture Specification
...
Unified fusion architecture (AudioEncoder + GapEncoder + FusionTransformer, single cursor-token output). This experiment uses exponential decay ramps (sharp spikes at event positions) instead of linear ramps, plus random amplitude jitter during training.

### Mel-Embedded Event Ramps (exponential decay, full-band)

Called as `_embed_events_in_mel()` at the start of `forward()`. Fully vectorized.

```
For each frame t, compute "time since last event" as an exponential decay:
  ramp(t) = exp(-ln(2) * elapsed / half_life)
  where:
    elapsed = t - last_event_before_t (clamped >= 0)
    half_life = 0.03 * gap_to_next_event (clamped >= 0.5 frames)

Result: sharp spike (1.0) at each event position, decaying to ~0.5 at 3% of the gap.
Zero where no valid events exist before frame t.

Audio scaling:
  Training: mel = mel * random(0.25, 0.75) per sample + ramp * 10.0
  Eval:     mel = mel * 0.5 + ramp * 10.0
  Ramp added uniformly to all 80 mel bands.
```

The exponential decay makes event positions sharp and unambiguous (unlike linear ramps). Amplitude jitter (0.25-0.75x) prevents the model from learning a fixed ramp-to-audio ratio.
...
#### Conv Stem (mel with exponential ramps → audio tokens)
...
Applied after conv stem, after every encoder layer, and every fusion layer:
```
...
### Audio Augmentation (applied to mel_window, before ramp embedding)
| Aug | Rate | Params |
|---|---|---|
| Mel gain | 30% | +/-2dB |
...
### Mel Ramp Augmentation (built into _embed_events_in_mel)
| Aug | Rate | Params |
|---|---|---|
...
## Key Metrics (ATH at eval 8)

| Metric | Value |
|---|---|
| HIT (<=3% or +/-1 frame) | 71.6% |
| MISS (>20% off) | 27.9% |
| Exact match | 52.7% |
| Context delta | 4.5% |
| no_events accuracy | 48.1% |
| Val loss | 2.533 |
| Unique predictions | 436 |
