# Experiment 34 — Full Architecture Specification
...
Gap tokens are summarized into a conditioning vector via attention pooling, then applied as FiLM modulation on the audio fusion layers. Context modulates how audio features are interpreted rather than competing with audio for attention.
...
Summarizes the C gap tokens into a single context conditioning vector:

```
query = learned_parameter (1, 1, 384), expanded to (B, 1, 384)
ctx_out = MultiheadAttention(384, 8, dropout=0.1)(
    query=query, key=gap_tokens, value=gap_tokens, key_padding_mask=gap_mask
)  → (B, 1, 384)
ctx_vec = Linear(384, 64) → GELU → Linear(64, 64) → LayerNorm(384 on input) → (B, 64)
```

### 5. Audio Fusion (4 layers, dual FiLM)

Audio-only self-attention with both density and context FiLM conditioning:
...
for each of 4 layers:
    audio = TransformerEncoderLayer(
        d_model=384, nhead=8, dim_feedforward=1536,
        dropout=0.1, activation="gelu", batch_first=True, norm_first=True
    )(audio)                              # audio self-attention (250 tokens only)
    audio = density_FiLM(audio, cond)     # density modulation
    audio = context_FiLM(audio, ctx_vec)  # context modulation
```

Each fusion layer has two independent FiLM modules:
- `density_FiLM`: modulates from density conditioning (cond, dim=64)
- `context_FiLM`: modulates from context vector (ctx_vec, dim=64)
...
Standard FiLM applied at multiple points:
```
cond (B, 64) → Linear(64, 2*384) → split → scale (B, 384), shift (B, 384)
x = x * (1 + scale) + shift
```

Initialized to identity (zero weights/bias).
...
| Metric | Value |
|---|---|
| HIT (<=3% or +/-1 frame) | 66.5% |
| MISS (>20% off) | 32.8% |
| Context delta | 4.2% |
| no_events accuracy | 43.6% |
| Unique predictions | 467 |
