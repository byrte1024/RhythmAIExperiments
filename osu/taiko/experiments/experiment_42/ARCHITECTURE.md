# Experiment 42 — Full Architecture Specification
...
Single-pathway architecture: event context is injected by adding learned embeddings directly to audio tokens at event positions, then processed through unified self-attention. No separate context tokens, no cross-attention, no mel modification.
...
For each of 128 context events, compute 3 embedding inputs:
- **Presence embedding**: learned parameter (1, 384)
- **Gap before**: sinusoidal encoding of distance from previous event (first event defaults to 50.0)
- **Gap after**: sinusoidal encoding of distance to next event (last valid event uses gap_before as proxy to avoid target leakage)
...
**No gap ratios** — this version uses 3 embedding inputs (presence + gap_before + gap_after), not the 5 inputs (+ gap_ratio_before + gap_ratio_after) added in later experiments.
...
| Batch size | 48 |
| Epochs | 50 (killed after eval 9, ~3.25 epochs) |
...
## Key Metrics (ATH at eval 9)

| Metric | Value |
|---|---|
| HIT (<=3% or +/-1 frame) | 73.2% |
| MISS (>20% off) | 26.4% |
| Score | 0.379 |
| Val loss | 2.481 |
| Frame error | 11.0 |
| no_events acc | 50.5% |
| no_audio acc | 39.3% |
| Context delta (acc - no_events acc) | 3.8pp |
| Metronome benchmark | 25.4% |

## Key Differences from Exp 44

This was the first EventEmbeddingDetector experiment. Compared to exp 44:
- **3 embedding inputs** (presence + gap_before + gap_after) instead of 5 (no gap_ratio_before, no gap_ratio_after)
