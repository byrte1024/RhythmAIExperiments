...
Experiments [31](../experiment_31/README.md)-[32](../experiment_32/README.md) revealed a fundamental tension in dual-stream architectures:
- **Exp [31](../experiment_31/README.md)** (late cross-attention): 18.8% context delta but severe banding (53 unique predictions) — cross-attention overwrites fine-grained audio features
- **Exp [32](../experiment_32/README.md)** (+ audio skip connection): 363 unique predictions but -1.8% context delta — skip connection lets the model bypass cross-attention entirely
...
- **vs Unified (exp [25](../experiment_25/README.md)-[27](../experiment_27/README.md))**: audio and gap have dedicated self-attention, gap tokens can't be drowned out
- **vs Dual-stream (exp [31](../experiment_31/README.md))**: cross-attention is interleaved, not bolted on at the end. Audio preserves fine-grained features between cross-attention injections
- **vs Skip connection (exp [32](../experiment_32/README.md))**: no audio shortcut that bypasses context. Context is woven into every layer
...
- 12 attention layers total is deeper than prior architectures (10 for unified (exp [25](../experiment_25/README.md)-[27](../experiment_27/README.md))/dual-stream (exp [31](../experiment_31/README.md))). May be slower to train and harder to optimize.
- Cross-attention at every layer may still see gap activations overwhelming audio. The magnitude imbalance (gap ±20 vs audio ±7) from exp [31-B](../experiment_31b/README.md) could recur.
- May need the activation clamping safety net from exp [31-B](../experiment_31b/README.md).
...
Compare to exp [31](../experiment_31/README.md) (late cross-attention): 44.9% HIT at eval 1 because audio had 4 self-attention layers to bootstrap before cross-attention. Here, cross-attention disrupts audio from layer 1.
...
- **Three cross-attention experiments ([31](../experiment_31/README.md)-33) all fail differently but for related reasons.** Late fusion → banding. Skip connection → context bypass. Interleaved → cold start. The cross-attention mechanism itself is the wrong tool for fusing fine-grained audio with coarse context.
