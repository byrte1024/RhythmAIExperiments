# Experiment 31-B — Full Architecture Specification
...
Dual-stream architecture: two independent streams process audio and context in parallel, then exchange information via bidirectional cross-attention. This variant uses 4 cross-attention layers instead of 2.
...
### 4. Cross-Attention Fusion (4 layers)

Each CrossAttentionFusionLayer performs bidirectional cross-attention:

```
for each of 4 CrossAttentionFusionLayer:
...
| Metric | Value |
|---|---|
| HIT (<=3% or +/-1 frame) | 18.6% |
| MISS (>20% off) | 60.1% |
| Context delta | 2.0% |
| no_events accuracy | 4.9% |
| Unique predictions | 37 |
