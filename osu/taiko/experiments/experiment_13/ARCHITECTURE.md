# Experiment 13 — Full Architecture Specification
...
Two-path architecture reverted to exp 11's exact architecture. Only change vs exp 11 is the addition of AR-simulating augmentations (no architecture or loss changes).
...
| Batch size | 256 |
| Epochs | 2 (stopped early — data alignment bug discovered) |
...
AR-simulating augmentations applied to the two-path architecture:
...
### Data Alignment Bug (discovered during this experiment)

Mel frame duration = HOP_LENGTH / SAMPLE_RATE = 110 / 22050 = 4.98866ms, but BIN_MS was hardcoded to 5.0ms. The 0.01134ms-per-frame error compounds progressively:
...
This misalignment limited all experiments 10-13 to a ~46% accuracy ceiling.
...
| val_loss | 3.009 |
| accuracy | 37.8% |
| HIT (<=3% or +/-1 frame) | 54.9% |
| STOP F1 | 0.325 |
| p99 error | 316 frames |
| no_events acc | 34.0% |
| no_audio acc | 0.4% |
| Metronome benchmark | 30.6% |
| time_shifted | 33.2% |
| random_events | 32.5% |

## AR Augmentation Effectiveness

The AR augmentations worked as intended:
- no_audio dropped to 0.4% — strongest audio dominance ever observed
- Corruption resilience up ~8-10pp across the board vs exp 11 (metronome 30.6% vs 22.1%, time_shifted 33.2% vs 23.0%, random_events 32.5% vs 26.0%)
- The model learned that event context can be noisy and should not be blindly trusted

However, the accuracy ceiling (~46%) was identified as a data problem (BIN_MS misalignment), not a model problem. The dataset was regenerated with correct BIN_MS for subsequent experiments.
