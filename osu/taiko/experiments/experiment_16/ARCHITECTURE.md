# Experiment 16 — Full Architecture Specification
...
Two-path architecture. Both paths produce 501-way logits, which are summed. This experiment uses rank-weighted per-sample weighting for the context auxiliary loss instead of a flat 0.1 multiplier.
...
Three-term loss: main logits + audio auxiliary + rank-weighted context auxiliary.

```
loss = OnsetLoss(logits, targets) + 0.2 * OnsetLoss(audio_logits, targets) + rank_weighted_context
```

### Rank-Weighted Context Loss

Per-sample weighting based on how highly audio ranked the true target:

```
rank = position of true target in audio_logits.argsort(descending=True)
weight = clamp(5 / (rank + 4), min=0.1, max=1.0)
rank_weighted_context = mean(weight * OnsetLoss(context_logits, target))
```

| Audio rank of target | Weight |
|---|---|
| 1 | 1.0 |
| 2 | 0.83 |
| 3 | 0.71 |
...
Weights computed from `audio_logits` under `no_grad()` — no feedback loop. Bounded 10x max/min ratio. No outer multiplier — the weighting itself controls magnitude.
...
| Batch size | 256 |
| Epochs | 2 (stopped — context actively harmful) |
...
| Metric | Value |
|---|---|
| val_loss | 2.946 (increasing from E1's 2.865) |
| accuracy | 40.2% |
| HIT (<=3% or +/-1 frame) | 61.2% |
| MISS (>20% off) | 36.1% |
| STOP F1 | 0.386 |
| p99 error | 204 frames |
| no_events acc | 41.4% |
| no_audio acc | 0.3% |
| Metronome benchmark | 41.5% |
| zero_density | 6.0% |
| random_density | 33.9% |
