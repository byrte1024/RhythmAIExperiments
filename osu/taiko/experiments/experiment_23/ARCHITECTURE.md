# Experiment 23 — Full Architecture Specification

## Task

Predict the next onset timing in an osu!taiko rhythm game chart, given audio + past event context. Autoregressive: each prediction advances the cursor to the predicted onset position. Context path reranks audio's top-K candidates using gap-based rhythm features, with candidates shuffled and confidence scores restored.
...
Two-path architecture: a frozen audio path produces 501-way logits, a trainable context path reranks the top-20 candidates. Candidates shuffled during training with softmax probability as a per-candidate confidence feature.
...
### 5. RerankerContextPath (TRAINABLE) — shuffled gap-based K-way reranker with confidence

Context operates entirely in "gap space." Uses its own encoders (d_ctx=192). Candidates shuffled; each gets softmax probability as a confidence feature (but no rank ordering).
...
#### 5b. Candidate Building — shuffled top-K with confidence (no rank)
...
For each of K=20 candidates:
  ...
  audio_probs = softmax(audio_logits).gather(top_k_indices) → (B, 20)
  score_feat = score_proj: Linear(1, 192) → GELU → (B, 20, 192)
    input: softmax probability per candidate (single scalar, no rank)

candidate_feat = candidate_combine: Linear(192*3, 192) → GELU → LayerNorm(192) → (B, 20, 192)
```

**Candidate shuffle during training:** Per sample, the K=20 candidates are randomly permuted. Context sees confidence magnitude but not which candidate was audio's top pick.
...
No baseline comparison, no miss penalty, no asymmetric scaling. Audio loss: none (audio path is frozen).
...
| Delta | -3.2pp |
| Override rate | 53.9% |
| Override accuracy | 63.7% |
| Override F1 | 67.0% |
| true_topK:false_topK | 34.3%:8.6% (4:1 ratio) |
