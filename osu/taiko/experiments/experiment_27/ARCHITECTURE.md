# Experiment 27 — Full Architecture Specification

## Task

Predict the next onset timing in an osu!taiko rhythm game chart, given audio + past event context. Autoregressive: each prediction advances the cursor to the predicted onset position. Single unified model — audio and gap features fused via self-attention. Full dataset (no subsampling).
...
| Batch size | 48 (reduced from 64 for memory with full dataset) |
| Epochs | ~3.5 (killed after eval 10) |
| Scheduler | CosineAnnealingLR |
| Subsample | 1 (full dataset, ~600K samples vs ~150K with subsample=4) |
| Evals per epoch | 4 (each eval = ~1 subsample-4 epoch of training data) |
...
- Full dataset: ~600K training samples (4x more than subsample=4)
...
## Key Metrics (eval 8 / epoch 2.0 best)

| Metric | Value |
|---|---|
| HIT (<=3% or +/-1 frame) | 69.8% |
| MISS (>20% off) | 29.8% |
| Accuracy | 51.5% |
| Score | 0.343 |
| Stop F1 | 0.525 |
| Frame error mean | 11.5 |
| no_events acc | ~50% |
| Context delta | 1.5% |
| Val loss | 2.560 |
...
Broke the ~69% HIT ceiling for the first time (69.8%), with new highs across most metrics. But context contribution still collapsed: 8.1% (eval 1) to 1.2-2.4% (eval 4+). More diverse gap histories did not prevent audio-dominant convergence. The ~70% ceiling was confirmed as architectural, not data-limited. Systematic errors were 2x ratio-based over-prediction — the model predicts rhythmically valid candidates (96% top-10) but cannot disambiguate between them.
