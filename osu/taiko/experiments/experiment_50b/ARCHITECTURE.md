# Experiment 50-B — Full Architecture Specification
...
### Anti-Entropy (entropy_weight=0.5)
After computing onset_loss:
```
probs = softmax(logits)
entropy = -sum(probs * log(probs + 1e-10), dim=-1).mean()
loss = onset_loss + 0.5 * entropy
```
At entropy ~2.3 nats, this adds ~1.15 to the loss (~40% of onset loss). Strongly incentivizes peaked distributions.

**Key effect:** Eliminates the trimodal entropy distribution (peaks at 1.4, 2.3, 3.0 nats) and creates a bimodal distribution (peaks at 0.8 and 1.7 nats). The "disambiguation zone" at 2.3 nats disappears.
...
| HIT | 72.5% |
| MISS | 27.0% |
| Metronome benchmark | 46.5% |
| Adv metronome | 51.7% |
| Entropy structure | Bimodal (eliminated disambiguation zone) |
