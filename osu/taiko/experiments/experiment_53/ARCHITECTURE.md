# Experiment 53 — Full Architecture Specification
...
This experiment introduces the B_AUDIO/B_PRED split: the model sees more future audio than it can predict into.

## Input

| Input | Shape | Description |
|---|---|---|
| mel | (B, 80, 750) | Mel spectrogram, 80 bands, 750 frames (250 past + 500 future at ~5ms/frame = 3.75s window) |
...
|---|---|---|
| A_BINS | 250 | Past audio context (1.25s) |
...
| WINDOW | 750 | Total mel frames (A + B_AUDIO) |
| Tokens | 187 | Conv stem output (750 // 4) |
| Cursor token | 62 | A_BINS // 4 |
...
mel (B, 80, 750)
  ...
  → transpose → (B, 187, 384)
  → LayerNorm(384)
  → + SinusoidalPosEmb(positions 0..186)
  → FiLM(cond)
  → x: (B, 187, 384) audio tokens
```
4x downsample: 750 mel frames → 187 tokens. Cursor at token 62.

The model has audio tokens covering bins -250 to +500 relative to cursor. Tokens 0-61 are past audio, token 62 is the cursor, tokens 63-186 are future audio. The prediction head outputs 251 classes (bins 0-249 + STOP), but the transformer sees audio up to bin +500 — giving 250 extra bins of future spectral context beyond the prediction range.
...
→ event_emb: (B, 128, 384)
```
Events are mapped to audio token positions: `token = (250 + offset) // 4`. Only events mapping to tokens 0-61 (past audio) are used. Event embeddings are scatter-added to the corresponding audio tokens.
...
for each of 8 layers:
    ...
    x = FiLM(cond)(x)
```
187 tokens attend to each other. Future audio tokens (63-186) provide spectral context even though the prediction head can only output bins 0-249.

### 5. Output Head
```
cursor = x[:, 62, :]  # cursor token (B, 384)
...
2. Extract mel window: cursor - 250 to cursor + 500 (750 frames total)
...
The model sees 2.5s of future audio at each step but only commits to predictions within the next 1.25s. Audio beyond the prediction range provides spectral context (upcoming transients, energy patterns) without requiring the model to classify into 500+ bins.
