# Experiment 07 — Full Architecture Specification
...
Single-path architecture: AudioEncoder produces audio tokens, EventDecoder cross-attends to audio and produces logits via a query token. Identical model architecture to experiments 05 and 06.
...
Trapezoid soft targets with hard cutoffs (no Gaussian tails).
...
**STOP weight increased to 3.0** (3x penalty for missing STOP predictions) to address the model's reluctance to predict STOP observed in exp 06.
...
| Batch size | 256 |
| Epochs | 1 (killed early after complete collapse) |
...
## Augmentation (HEAVY — Key Change)

**The key change in this experiment was dramatically increasing context augmentation rates** to force the model to rely on audio instead of events:

| Aug | Rate | Description |
|---|---|---|
| Context dropout | **25%** (was 8%) | Remove all past events entirely |
...
In total, ~25% of samples had no events and another ~25% had warped/shuffled events, meaning roughly half of all training samples had corrupted or absent event context.
...
|---|---|
| Accuracy | 8.1% |
| HIT (<=3% or +/-1 frame) | 17.5% |
| MISS (>20% off) | 63.1% |
| STOP F1 | 0.041 |
| Frame error median | 12.0 |
