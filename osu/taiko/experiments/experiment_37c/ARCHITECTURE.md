# Experiment 37-C — Full Architecture Specification

## Task

Predict ALL onsets in the forward audio window using per-bin sigmoid outputs with focal dice loss.
...
```
gap_features = SinusoidalPosEmb(384)(all_gaps.abs())
snippet_feat = snippet_encoder(mel_snippet_at_event_pos)
...
for each of 2 layers:
    x = TransformerEncoderLayer(d_model=384, nhead=8, ...)(x, mask=gap_mask)
    x = FiLM(cond)(x)
```
...
Dice loss (90% weight) + BCE (10% weight) for stable gradients.
...
smooth=1.0 prevents division by zero. Dice directly optimizes set overlap.

### Combined Loss

```
loss = 1.0 * dice_loss + 0.1 * BCE_loss
```

No pos_weight, no focal_gamma on the BCE component.
...
Killed early. Train HIT=5.4%, miss=93.7% at ~15% through epoch 1.
