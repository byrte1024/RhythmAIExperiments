# Experiment 36-B — Full Architecture Specification

## Task

Predict ALL onsets in the forward audio window of an osu!taiko rhythm game chart, given audio + past event context. At inference, the earliest prediction above a confidence threshold is selected.
...
## Loss: MultiTargetOnsetLoss (with per-onset recall loss)
...
### Per-Onset Recall Loss

For each real onset at bin `onset_i`, compute `-log(softmax_prob[onset_i])`. This directly penalizes missing any individual onset regardless of how many others exist.

### Combined Loss

```
loss = 0.3 * hard_CE(nearest_target)
     + 0.7 * soft_CE(multi_target_distribution)
     + 1.0 * mean(-log(prob[onset_i]) for each real onset)
```

recall_weight = 1.0, equal weight to the CE terms.
...
6. If no bin above threshold: hop forward (STOP equivalent)
