# Experiment 11 — Full Architecture Specification
...
Two-path architecture: an audio path and a context path each produce 501-way logits, which are summed. Identical to exp 10 except for the NaN guard in EventEncoder.
...
NaN guard: if all events are masked for a sample, the last position is unmasked as a dummy. This prevents softmax over all -inf producing NaN.
...
|---|---|
| val_loss | 2.747 |
| HIT (<=3% or +/-1 frame) | 64.8% |
| accuracy | 47.1% |
| STOP F1 | 0.398 |
| p99 error | 180 frames |
| no_events acc | 36.8% |
| no_audio acc | 15.5% |
| top-3 HIT | ~86% |
| top-10 HIT | ~95% |
| Metronome benchmark | 23.5% |
