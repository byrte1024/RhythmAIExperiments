# Experiment 35 — Full Architecture Specification
...
## Model: OnsetDetector (with edge-band mel ramps)

**Total parameters: ~19M**

Unified fusion architecture (AudioEncoder + GapEncoder + FusionTransformer, single cursor-token output), with event ramps embedded in reserved mel bands.

### Mel-Embedded Event Ramps (edge-band)

Before audio encoding, linear gradient ramps are embedded in reserved mel bands at event positions:
...
For each pair of consecutive events (and from last event to cursor):
  ...
  Encoded in reserved mel bands with fading intensity:
    Band 0  (bottom): 100% ramp intensity (10 dB)
    Band 1:            50%
    Band 2:            25%
    Band 77:           25%
    Band 78:           50%
...
The ramp creates a sawtooth pattern whose frequency encodes the rhythm pattern. Slope = 1/gap_length.
...
#### Conv Stem (mel with ramps → audio tokens)
...
| Metric | Value |
|---|---|
| HIT (<=3% or +/-1 frame) | 65.7% |
| MISS (>20% off) | 33.7% |
| Context delta | 5.0% |
| no_events accuracy | 42.8% |
| Unique predictions | 449 |
